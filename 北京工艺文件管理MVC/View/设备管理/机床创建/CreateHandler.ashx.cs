            ...
            using (Model1  pm1 = new Model1 ())
            {
               ...
               // var state = pm1.JDJS_WMS_Device_Status_Table.Where(r => r.ID == stateId).FirstOrDefault().Status;
                var use2 =
                        ...
                        from JDJS_WMS_Device_Type_Infos in pm1.JDJS_PDMS_Device_Type_Info 
                ...
                var InspectIp = from ips in pm1.JDJS_PDMS_Device_Info 
                                where ips.IP == ip
                                select new
                                {
                                    ips.IP
                                };
                var InspectNumber = from numbers in pm1.JDJS_PDMS_Device_Info 
                ...
                if (InspectIps == "[]" && InspectNumbers == "[]")
                {
                    foreach (var item in use2)
                    {
                        ...
                        var user1 = new JDJS_PDMS_Device_Info_Row () { MachNum = number, IP = ip, TypeID = use4, LocationID =int.Parse(id) };
                        pm1.JDJS_PDMS_Device_Info .Add(user1);
