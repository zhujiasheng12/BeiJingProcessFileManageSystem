...
namespace WebApplication2.Model
{
    ...
    public class modifyHandler1 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (Model1  pm4 = new Model1 ())
            {
               // var machState = pm4.JDJS_WMS_Device_Status_Table.Where(r => r.ID == stateId).FirstOrDefault().Status;
                var user = 
                           from JDJS_WMS_Device_Brand_Infos in pm4.JDJS_PDMS_Device_Brand_Info 
                           from JDJS_WMS_Device_Type_Infos in pm4.JDJS_PDMS_Device_Type_Info
                ...
                var list = from JDJS_WMS_Device_Infos in pm4.JDJS_PDMS_Device_Info 
                           select new
                           {
                ...
                if (InspectIps == "[]" && InspectNumbers == "[]")
                {
                    ...
                    var users = pm4.JDJS_PDMS_Device_Info .Where(r => r.ID == id);
                    foreach (var item in users)
                    {
                        item.IP = ip; item.MachNum = number;item.LocationID  = position;
