    ...
    public class BrandDeleteHandler1 : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (Model1  pm2 = new Model1 ())
            {
                ...
                var model = pm2.JDJS_PDMS_Device_Type_Info.Where(r => r.BrandID == id);
                if (model.Count() > 0)
                {
                ...
                {
                    var user = pm2.JDJS_PDMS_Device_Brand_Info .Where(r => r.ID == id).First();
                    pm2.JDJS_PDMS_Device_Brand_Info .Remove(user);
                    pm2.SaveChanges();
